...
#HTML문서를 넘겨주는 것이 아닌 리다이렉트 주소를 넘겨줄 때 HttpResponseRedirect 클래스의 객체를 반환하면 됨
#HttpResponseRedirect(사용자에게 넘겨줄 주소(문자열))

#reverse : 템플릿의 url 태그와 동일한 기능을 수행. 별칭을 기반으로한 URL 검색
...
def index(request):
    ...
    #HTML문서에 객체 리스트 전달 후 클라이언트에게 HTML파일 전송
...
def detail(request, qid):
    ...
    #조건에 맞는 객체가 한개도 없는경우, 사용자가 잘못요청한 것으로 처리해 404에러를 띄우는 처리를 해줌
...
def vote(request):
    ...
    if request.method == "POST":
        ...
        c.votes += 1  #투표 수 증가 c.votes = c.votes + 1
        c.save() #변동사항을 데이터베이스에 반영
        
        #vote어플리케이션의 index뷰로 이동을 할 수 있도록 URL주소를 넘겨줌
        #c.q : Choice 객체 c에 저장된 q를 접근 -> c와 연결된 Question객체 자체를 의미
        #c.q.id : Choice 객체c와 연결된 Question 객체의 id변수값을 가져옴
...
#데코레이터 : URL 요청에 따라 뷰함수를 호출하는 데, 호출전에 데코레이터가 먼저 실행되서
#          초기화작업, 로그인확인, 접근권한 확인을 수행하는 기능
#함수에 데코레이터 붙이기
#@데코레이터 함수 이름
#def func1()

#클래스에 데코레이터 붙이기
#class Class(사용할 데코레이터클래스명):

#login_required : 뷰함수 호출 전 요청을 한 클라이언트가 로그인을 했는지 여부를 파악하고
# 비로그인상태인 경우 로그인페이지를 띄워주는 데코레이터 함수
...
def qregister(request):
    ...
    if request.method == "GET":
        #모델폼클래스 객체 생성
        #모델폼클래스 객체 생성 시 매개변수에 값을 전달하지 않는경우, 입력양식에 값이 비어있는형태로
        #객체가 생성됨 
        form = QuestionForm()
        #HTML 파일 전달
    ...
    #POST방식 처리
        ...
        if form.is_valid() : #사용자가 입력한 값이 유효한 값인지 확인
            #폼객체.cleaned_data : is_valid()함수로 유효한 값인지 확인 후 True값을 반환했을때,
            #사용자의 입력정보를 확인할 수 있는 사전형 변수
            #폼객체.save() : 모델폼클래스의 객체만 사용가능. 연동된 모델클래스의 객체로 변환 후 데이터베이스에 저장
            #폼객체.save(commit=False) : 연동된 모델클래스의 객체로 변환만 해줌
            
            #Question 객체는 date 변수에 값이 비어있으면 안되므로 파이썬 코드로 date변수값을 채운 후 DB에저장해야함
            q = form.save(commit=False)#q : form에 저장된 값을 바탕으로 생성된 Question객체 
            print('생성된 question 객체', q)
            q.date = datetime.now() #파이썬 모듈을 이용해 현재시간을 생성된 Question객체의 date변수에 저장
            q.save() #해당Question객체를 데이터베이스에 저장함
...
def cregister(request):
    if request.method == "GET":
        ...
        else: #사용자의 입력이 유효한 값이 아닌경우
            #사용자가 입력한 데이터를 바탕으로 생성된 form객체와 error변수를 HTML파일에 전달
            return render(request, 'vote/cregister.html', 
                          {'f' : form , 'error':'유효하지 않는 값입니다.'})
...
def qupdate(request, q_id):
    ...
    if request.method == "GET":
        ...
        #기존 객체에 저장된 변수값을 사용자의 입력 데이터로 변경한 모델폼 객체를 생성
...
def qdelete(request, q_id):
    ...
    q.delete()
    return HttpResponseRedirect(reverse('vote:index'))
#Choice 객체 삭제 요청 처리 뷰
#1)매개변수를 추가(삭제하고자하는 Choice객체의 id값)   
@login_required
