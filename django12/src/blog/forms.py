...
#글 등록에 사용할 폼 클래스 - 모델 폼 클래스 상속
class PostForm(ModelForm):
    #사용자의 첨부파일, 이미지 업로드에 사용할 <input>을 커스텀으로 설정
    #required=False : 해당 <input>을 사용자가 필수로 입력하지 않아도 되는 공간 설정
    #ClearableFileInput : <input type='file'> 형태의 입력 공간에 추가 설정을 할 때 사용하는 위젯
    #attrs = {'multiple':True} : 하나의 입력 공간에 여러 개의 파일을 업로드할 수 있도록 설정
