...
class Post(models.Model):
    ...
    #models.SET_NULL : 연결된 객체가 삭제되면 null 값을 저장
    ...
    #models.TextField : 글자 수 제한이 없는 문자열 저장 공간
    #null(기본값 False) : 데이터베이스에 객체를 저장할 때 해당 변수값이 비어 있어도 생성되도록 허용 - true로 변경하면
    #blank(기본값 False) : 폼객체.is_valid(), 폼객체.as_p를 사용 했을 때 사용자가 입력하지 않아도 되도록 허용 - true로 변경하면
    content = models.TextField('내용', null = True, blank = True)
    #auto_now_add : 객체가 생성될 때 자동으로 서버의 현재 시간을 저장하도록 설정
...
class PostImage(models.Model):
    ...
    #ImageField : 이미지가 저장되는 경로를 저장하는 공간
    #upload_to : 실제 이미지를 저장할 때 사용할 경로
    # %Y : 해당 서버의 현재 년도 %m : 해당 서버의 현재 월, %d : 해당 서버의 현재 일
    
    #객체 삭제 시 호출되는 함수. 객체 삭제 전 실제 이미지파일을 삭제하는 코드를 삽입
...
class PostFile(models.Model):
    ...
    #FileField : 파일(이미지,실행파일,엑셀,워드 등)의 경로를 저장하는 공간
